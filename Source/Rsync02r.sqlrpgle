...
// Invio lo script sul sistema remoto e lo eseguo
Parm1 = '/QOpenSys/pkgs/bin/bash' + X'00';
Parm2 = '-c' + X'00';
Parm3 = 'chmod 775 ' + setccsidscript + X'00';

Qp2Shell(Parm1 :Parm2 :Parm3);

Parm3 = 'scp -P ' + %Char(RemotePort) + ' ' + CertificatePath + ' ' +  setccsidscript + ' ' + 
          RSyncUser + '@' + %Trim(RemoteServer) + ':' + setccsidscript + X'00';

Qp2Shell(Parm1 :Parm2 :Parm3);

Parm3 = 'ssh -p ' + %Char(RemotePort) + ' ' + CertificatePath + ' ' + 
          RSyncUser + '@' + %Trim(RemoteServer) + ' ' + setccsidscript + X'00';
...
Dcl-Proc CheckDirectory;
  ...
  Dow SqlState <> '02000' and not %Shtdn();
    ...
    // // set CCSID
    // Parm3 = 'ssh ' + ToSite + ' -o UserKnownHostsFile=' + UserKnownHosts + ' -i ' 
    //         + Certificate + ' -p' + %Char(RemotePort) + 
    //          ' "setccsid -h "' + WorkCcsid + '" ' + WorkStmf + '"' + X'00';
    Parm3 = 'echo "setccsid -h ' + WorkCcsid + ' ' +A+ WorkStmf +A+ '" >> ' + setccsidscript + X'00';

    Qp2Shell(Parm1 :Parm2 :Parm3);

    RSyncLog(INFO :'Setccsid: for ' + WorkStmf + ' to: ' + WorkCcsid + '.');
...
Dcl-Proc RSyncSetup;
  ...
  Parm3 = *AllX'00';
  Parm3 = 'export PATH="/QOpenSys/pkgs/bin:/QOpenSys/usr/bin:/usr/ccs/bin:/QOpenSys/usr/bin/X11:/usr/sbin:.:/usr/bin"' + X'00';
  Qp2Shell(Parm1 :Parm2 :Parm3);
  ...
  setccsidscript   = Directory + '/' + 'setccsidscript.sh';

  Rc = Cmd('del Objlnk(' + A + setccsidscript + A + ')');
